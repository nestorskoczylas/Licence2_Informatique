...
# Pour cela il faut taper la commande suivante dans une console shell :
...
phi1 = [[encode(i,j,k) for k in range(9)]
        for i in range(9)
        for j in range(9)]
...
phi2 = [[-encode(i,j,k1), -encode(i,j2,k2)]
    for i in l
    for j1 in l
    for j2 in l
    for k in l
...
phi5 = [[-encode(i,j,k),-encode(i2,j2,k)]
        for i in range(9)
        for j in range(9)
        for k in range(9)
        for i2 in range(9)
