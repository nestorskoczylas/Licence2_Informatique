...
function authorToOption(array $authors) : string {
	return "<option value='".$authors["id"]."'>".$authors["nom"]."</option>";
}

function authorsArrayToOptions(array $authorsArray) : string {
	$rep = "";
	foreach ($authorsArray as $value) {
		$rep .= authorToOption($value)."\n";
	}
	return $rep;
}

function yearToOption(array $years) : string {
    return "<option value='".$years["id"]."'>".$years["annee"]."</option>";
}

function yearsArrayToOptions(array $yearsArray) : string {
    $rep = "";
    foreach ($yearsArray as $value) {
        $rep .= yearToOption($value)."\n";
    }
    return $rep;
}
