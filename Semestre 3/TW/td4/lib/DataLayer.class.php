...
EOD;
	...
	function getBooksByYear(int $year) : array {
		$sql = <<<EOD
			select titre, annee, categorie, serie, couverture, auteurs
			from livres_complets
			where annee in (select annee from livres where annee=:year)
EOD;
		$stmt = $this->conn->prepare($sql);
...
/*
3 choses pour la note de TD
- y'aura un projet à faire, pas un gros, ça sera un TP un peu plus long
- y'a une note sur les gens qui ont rendu TOUS les TP sans regarder la qualité
- il sélectionne au hasard un TP parmi les TP rendus qui sera noté
et enfin un ds final
*/
?>
