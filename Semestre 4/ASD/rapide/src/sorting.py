...
def quicksort (t, cmp, piv):
    ...
    >>> quicksort(t,cmp,naive_pivot)
    >>> list(t)
    [1, 2, 3, 4, 5, 6, 7, 8, 9]

    """
    s= {'left':0,'right':len(t)-1,'data':t}
    quicksort_slice(s, cmp, piv)


def quicksort_slice (s, cmp, piv):
    """
    ...
    if s['left'] < s['right'] :
        p1,p2 = partition(s, cmp, piv(s,cmp))

        quicksort_slice(p1,cmp,piv)
        quicksort_slice(p2,cmp,piv)

    

def optimal_pivot(s,cmp):
    
    z = [i.value for i in s["data"][s['left']:s['right']]]
    a = round(np.median(z))
    b = element.Element(a)
    ...
    #return round(np.median([i.value for i in s["data"][s['left']:s['right']]]))
    


def random_pivot(s,cmp) : 

    """
    Returns a position for the pivot.
    Systematically returns a random position
...
def partition (s, cmp, pivot_pos):
    ...
    val = s['data'][s['right']]
    s['data'][s['right']] = pivot
    s['data'][pivot_pos] = val

    a = s['left']-1

    for b in range(s['left'],s['right']) :
        
        if cmp(s['data'][b],pivot) != 1 :
    ...
    val = s['data'][s['right']]
    s['data'][s['right']] = s['data'][a+1]
    s['data'][a+1] = val
    

    return ({'left' : s['left'], 'right' : a, 'data' : s['data']},
        {'left' : a+2, 'right' : s['right'], 'data' : s['data']})
